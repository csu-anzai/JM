...
Основой инкапсуляции является класс. Цель класса - инкапсуляция (сокрытие) сложности. Мы скрываем члены класса (поля, методы) от изменения из вне. Менять их можно только через методы.
